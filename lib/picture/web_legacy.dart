...
class _WebPictureState extends State<_WebPicture> {
  ...
  StreamSubscription<html.Event>? onLoadSubscription;
  StreamSubscription<html.Event>? onErrorSubscription;
  ...
  void changeToHtml(String src) {
    ...
    onLoadSubscription = image.onLoad.listen((_) {
      _loaded.value = true;
      setState(() {
        imageHeight = double.tryParse(image.height?.toString() ?? '');
        imageWidth = double.tryParse(image.width?.toString() ?? '');
      });
    });
